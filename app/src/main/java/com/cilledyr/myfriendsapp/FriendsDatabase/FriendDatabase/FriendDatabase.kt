...
val migration_7_8 = object : Migration(7, 8) {
    override fun migrate(database: SupportSQLiteDatabase) {
        database.execSQL(
            "ALTER TABLE BeFriend ADD COLUMN coordinateX DOUBLE NOT NULL DEFAULT ''"
        )
        database.execSQL(
            "ALTER TABLE BeFriend ADD COLUMN coordinateY DOUBLE NOT NULL DEFAULT ''"
        )
    }
}
    ...
    override fun migrate(database: SupportSQLiteDatabase) {
        ...
        database.execSQL(
            "CREATE TABLE friends_new " +
                    ...
                    "phoneNr TEXT NOT NULL, birthday INTEGER NOT NULL, isFavorite INTEGER NOT NULL," +
                    "PRIMARY KEY(id))");
        // Copy the data
        database.execSQL(
            "INSERT INTO friends_new (Id, firstName, lastName, email, phoneNr, birthday, isFavorite) SELECT " +
                    "Id, firstName, lastName, email, phoneNr, birthday, isFavorite FROM BeFriend");
